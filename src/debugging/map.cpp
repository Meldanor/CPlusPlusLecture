...
static void fillColumn(char* destination, unsigned int mapXCoordinate) {
	...
	int numSky = (whiteNoise[(mapXCoordinate / 16) % WHITE_NOISE_SIZE] * 4
			...
			+ whiteNoise[(mapXCoordinate / 2) % WHITE_NOISE_SIZE] / 2) / 100
			+ 3;

	for (int y = 0; y < MAP_SIZE_Y; ++y) {
		if (y < numSky)
			destination[y] = (y > 10) ? 2 : 0;
		else if (y < numSky + 1 && y <= 10)
			destination[y] = 1;
		else if (whiteNoise[mapXCoordinate % WHITE_NOISE_SIZE] / 15 == y - 5)
			destination[y] = 4;
		else
			destination[y] = 3;
	}
